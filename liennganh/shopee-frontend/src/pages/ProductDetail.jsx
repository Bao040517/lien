...
const ProductDetail = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (productData.category?.id) {
          api
            .get(`/products/category/${productData.category.id}`, {
              params: { size: 10 },
            })
            .then((res) => {
              const relatedData =
  ...
  const handleSelectOption = (attrName, value) => {
    ...
    if (product?.variants) {
      ...
      if (match) {
        ...
        if (match.imageUrl) setSelectedImage(match.imageUrl);
      }
    }
  };
  ...
  const handleChatNow = async () => {
    ...
    try {
      ...
      const convRes = await api.post(
        `/messages/conversations?userId1=${user.id}&userId2=${ownerId}`,
      );
      ...
      navigate("/messages", {
        state: { openConv: { id: conv.id, otherUser } },
      });
    } catch (e) {
  ...
  const displayImage =
    selectedImage || selectedVariant?.imageUrl || product.imageUrl;

  // Build all images list for gallery
  const allImages = (() => {
    const imgs = [];
      ...
      <div className="container mx-auto px-4 pt-6">
        ...
        <div className="bg-white rounded shadow-sm p-6 mb-4 flex flex-col md:flex-row gap-8">
          ...
          <div className="w-full md:w-2/5 flex-shrink-0">
            {/* Main image */}
            <div className="aspect-square bg-gray-100 rounded overflow-hidden border border-gray-200 mb-3">
              {displayImage ? (
                <img
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {allImages.map((img, idx) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded border-2 overflow-hidden transition-all duration-150 ${
                      displayImage === img
                        ? "border-primary-dark shadow-sm"
          ...
          <div className="flex-1">
            <h1 className="text-xl md:text-2xl font-medium mb-2 line-clamp-2">
              {product.name}
            </h1>
            ...
            <div className="flex items-center gap-4 mb-4 text-sm">
              ...
              <div className="border-l pl-4 border-gray-300 text-gray-500">
                <span className="text-black font-medium border-b border-black pb-0.5 mr-1">
                  {product.sold || 0}
                </span>
                Đã Bán
              </div>
              ...
              <div className="space-y-4 mb-6">
                {Object.entries(attributeGroups).map(([attrName, options]) => (
                  <div key={attrName} className="flex items-start gap-6">
                    <span className="text-gray-500 w-24 pt-2 shrink-0">
                      {attrName}
                    </span>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  disabled={
                    !quantity || quantity < 1 || quantity > displayStock
                  }
        ...
        <div className="bg-white rounded shadow-sm p-6 mb-4 flex items-center gap-4">
          ...
          <div>
            <div className="font-medium text-lg">
              {product.shop?.name || product.shopName || "Cửa hàng chính hãng"}
            </div>
            <div className="text-gray-500 text-sm">Online 5 phút trước</div>
            <div className="flex gap-2 mt-2">
              <button
                onClick={handleChatNow}
                className="px-3 py-1 border border-primary-dark text-primary-dark text-sm bg-primary-lighter rounded-sm flex items-center gap-1 hover:bg-primary-light transition"
              >
                <MessageSquare className="w-3 h-3" /> Chat Ngay
              </button>
              <Link
                to={`/shop/${product.shop?.id}`}
                className="px-3 py-1 border border-gray-300 text-gray-500 text-sm rounded-sm flex items-center gap-1 hover:bg-gray-50"
              >
        ...
        <div className="bg-white rounded shadow-sm p-6 mb-4">
          <div className="bg-gray-50 p-3 mb-4 text-lg uppercase text-gray-700 font-medium">
            Chi Tiết Sản Phẩm
          </div>
              ...
              <div className="text-center">
                ...
                <div className="flex items-center justify-center gap-0.5 mt-1">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Star
                      key={s}
                      className={`w-4 h-4 ${s <= Math.round(reviews.reduce((sum, r) => sum + (r.rating || 0), 0) / reviews.length) ? "fill-primary-dark text-primary-dark" : "text-gray-300"}`}
                    />
                  ))}
                </div>
              </div>
              <div className="flex gap-2 flex-wrap">
                {[5, 4, 3, 2, 1].map((star) => {
                  ...
                  return count > 0 ? (
                    <span
                      key={star}
                      className="text-xs border border-primary text-primary-dark px-3 py-1 rounded-full"
                    >
                      {star} Sao ({count})
                    </span>
                  ) : null;
                ...
                >
                  <div className="flex gap-3">
                    ...
                    <div className="w-10 h-10 bg-gradient-to-br from-primary-dark to-primary rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                      {(review.user?.username || review.username || "U")
                        .charAt(0)
                        .toUpperCase()}
                    </div>
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-0.5 my-1">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`w-3.5 h-3.5 ${i < (review.rating || 0) ? "fill-primary-dark text-primary-dark" : "text-gray-200"}`}
                          />
                        ))}
                      </div>
                      {/* Quality label */}
        ...
        {relatedProducts.length > 0 && (
          <div className="bg-white rounded shadow-sm overflow-hidden mb-8 mt-4 group/related relative">
            <div className="bg-gray-50 p-4 border-b">
              <h2 className="text-gray-700 uppercase font-medium text-lg">
            ...
            <div
              ref={relatedRef}
              className="overflow-x-auto snap-x flex scroll-smooth p-4 gap-4"
            ...
            >
              {relatedProducts.map((item) => (
                ...
                >
                  ...
                  <div className="p-3">
                    <h3 className="text-sm text-gray-800 line-clamp-2 min-h-[40px] mb-2">
                      {item.name}
                    </h3>
