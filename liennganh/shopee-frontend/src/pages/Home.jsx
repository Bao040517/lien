...
const CountdownTimer = ({ targetDate }) => {
  ...
  const format = (num) =>
    num !== undefined && num !== null ? num.toString().padStart(2, "0") : "00";
  ...
  return (
    <div className="flex gap-1 ml-4 text-white font-bold text-sm">
      <span className="bg-black p-1 rounded min-w-[24px] text-center">
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [productsRes, categoriesRes, flashSalesRes, bannersRes] =
        ...
        if (flashSalesRes.status === "fulfilled") {
          const flashSaleData =
            flashSalesRes.value.data?.data || flashSalesRes.value.data || [];
  ...
  const goToPage = (page) => {
    setCurrentPage(page);
    window.scrollTo({
      top: document.getElementById("daily-suggestions")?.offsetTop - 140,
      behavior: "smooth",
    });
  };
  ...
  return (
    <div className="bg-gray-100 min-h-screen pb-10">
      <div className="container mx-auto px-4 pt-4 space-y-6">
        {/* CI/CD TEST BANNER */}
        <div className="bg-green-500 text-white text-center py-2 px-4 rounded-lg font-semibold text-sm shadow">
          🚀 CI/CD hoạt động rồi nha! Deploy thành công!
        </div>
        ...
        <div className="bg-white p-4 rounded shadow-sm relative group/cats">
          <h2 className="text-gray-500 uppercase font-medium mb-4 text-sm">
            Danh Mục
          </h2>
          ...
          >
            ...
            {Array.from({ length: Math.ceil(categories.length / 20) }).map(
              (_, pageIndex) => {
                ...
                return (
                  <div
                    key={pageIndex}
                    className="min-w-full flex-shrink-0 snap-start grid grid-cols-4 md:grid-cols-10 gap-x-2 gap-y-4 px-2"
                  >
                    {pageCategories.map((cat) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-center text-gray-800 group-hover:text-primary-dark line-clamp-2 w-full">
                          {cat.name}
                        </span>
                      </Link>
                    ))}
        ...
        <div className="bg-white rounded shadow-sm overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-primary-darker via-primary to-primary-darker px-5 py-3 flex justify-between items-center">
            ...
            <Link
              to="/flash-sale"
              className="text-white text-sm font-medium flex items-center hover:opacity-80 transition"
            >
          ...
          <div className="relative py-4 px-3 group/flash">
            ...
            <div
              ref={flashSaleRef}
              className="overflow-x-auto flex gap-3 pb-2"
              style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
            >
              {(flashSales.length > 0 ? flashSales : products.slice(0, 12)).map(
                (item, idx) => {
                  ...
                  const salePrice = isFlashSaleItem
                    ? item.discountedPrice
                    : product.discountedPrice || originalPrice;
                  const discountPercent =
                    isFlashSaleItem && originalPrice > 0
                  ...
                  const productTotalStock =
                    (product.stockQuantity || 0) + (product.sold || 0);
                  const soldPercent =
                    isFlashSaleItem && stockTotal > 0
                      ? Math.min(
                          100,
                      ...
                      : productTotalStock > 0
                        ? Math.min(
                            100,
                            Math.round(
                              ((product.sold || 0) / productTotalStock) * 100,
                            ),
                          )
                        : 0;

                  const displaySoldCount = isFlashSaleItem
                    ? soldCount
                    : product.sold || 0;
        ...
        <div className="mt-8" id="daily-suggestions">
          <div className="bg-white sticky top-[120px] z-40 border-b-4 border-primary-dark py-3 text-center mb-4 shadow-sm">
            <h3 className="text-primary-dark font-bold uppercase text-lg">
              Gợi Ý Hôm Nay
            </h3>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {currentProducts.map((product) => (
              <Link
                to={toProductSlug(product.name, product.id)}
                key={product.id}
                className="bg-white rounded hover:shadow-lg hover:-translate-y-0.5 transition duration-100 border border-transparent hover:border-primary-dark cursor-pointer overflow-hidden relative block group"
              >
