...
const Checkout = () => {
    ...
    const handleApplyVoucher = async (shopId) => {
        ...
        try {
            ...
            toast.success(`Áp dụng thành công! Giảm: ${new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(res.data.data)}`);
        ...
        try {
            ...
            if (failures.length === 0) {
                ...
                if (results.length === failures.length) {
                    ...
                    toast.warning(`Đặt hàng hoàn tất một phần. ${results.length - failures.length} đơn thành công, ${failures.length} đơn thất bại.`);
                    refreshCart();
                    navigate('/');
                }
            }
