...
const AdminCategories = () => {
  ...
  const handleEdit = (cat) => {
    ...
    setImagePreview(cat.imageUrl ? getImageUrl(cat.imageUrl) : null);
    setImageFile(null);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setCategories((prev) =>
          prev.map((c) => (c.id === editingId ? res.data.data || res.data : c)),
        );
      } else {
        // Tạo mới kèm ảnh
      ...
      console.error(err);
      toast.info(
        editingId ? "Cập nhật danh mục thất bại!" : "Tạo danh mục thất bại!",
      );
    } finally {
      setSaving(false);
    }
  ...
  const confirmDelete = async (id) => {
    try {
      ...
      toast.info("Xoá danh mục thất bại!");
    }
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-gray-50/80 text-xs text-gray-500 uppercase border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium tracking-wider w-1/2">
                    Mô tả
                  </th>
                  <th className="px-6 py-4 text-center font-medium tracking-wider w-32">
                    Thao tác
                  </th>
                </tr>
              </thead>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl overflow-hidden border border-gray-100 bg-white flex items-center justify-center shadow-sm flex-shrink-0 group-hover:shadow-md transition-shadow">
                          ...
                          <Tag
                            className={`w-6 h-6 text-blue-400 ${cat.imageUrl ? "hidden" : ""}`}
                          />
                        </div>
                        <div>
