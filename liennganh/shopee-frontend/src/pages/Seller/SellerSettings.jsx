...
import { Store, Save, AlertCircle, CheckCircle, Upload } from "lucide-react";
import api from "../../api";
  ...
  const handleUpdateShop = async (e) => {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        finalAvatarUrl = uploadRes.data?.data || finalAvatarUrl;
      }
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Quản lý thông tin hiển thị của Shop với khách hàng
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <form onSubmit={handleUpdateShop} className="p-6">
          {message.text && (
            <div
              className={`p-4 rounded-lg mb-6 flex items-center gap-3 ${message.type === "success" ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"}`}
            >
          ...
          <div className="space-y-6">
                ...
                <div className="w-20 h-20 rounded-full overflow-hidden border border-gray-200 bg-gray-100 flex items-center justify-center">
                  {avatarUrl ? (
                    ...
                    <span className="text-xl font-bold text-gray-500">
                      {(shopName || user?.username || "S")
                        .charAt(0)
                        .toUpperCase()}
                    </span>
                  )}
                </div>
                <label className="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 text-sm text-gray-700">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-2">
                Tên Shop là thương hiệu của bạn, tên chuyên nghiệp sẽ giúp khách
                hàng tin tưởng hơn. (Tối đa 50 ký tự)
              </p>
            </div>
