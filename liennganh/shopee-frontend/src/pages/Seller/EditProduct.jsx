...
const EditProduct = ({ isAdmin = false }) => {
  ...
  useEffect(() => {
    fetchProduct();
    api
      .get("/categories", { params: { size: 1000 } })
      .then((res) =>
        setCategories(
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      for (const attr of newAttributes) {
        ...
        const attrRes = await api.post(`/products/${id}/attributes`, {
          name: attr.name,
        });
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {isAdmin && productInfo && (
        <div className="mb-6 p-3 bg-gray-50 rounded-lg border border-gray-200 text-sm text-gray-600 flex flex-wrap gap-4">
          <span>
            ID: <strong className="text-gray-800">{productInfo.id}</strong>
          </span>
          {productInfo.shop?.name && (
            <span>
              Shop:{" "}
              <strong className="text-gray-800">{productInfo.shop.name}</strong>
              {productInfo.shop.ownerUsername && (
          ...
          <span>
            Trạng thái:{" "}
            <strong className="text-gray-800">
              {productInfo.productStatus || "PENDING"}
            </strong>
          </span>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {existingImages.map((url, index) => (
              <div
                key={`existing-${index}`}
                className="relative w-24 h-24 rounded-lg overflow-hidden border-2 border-gray-200 group"
              >
                <img
                  src={getImageUrl(url)}
                  alt={`img-${index}`}
                ...
                {index === 0 &&
                  ...
                  newImageFiles.length === 0 && (
                    <span className="absolute top-0.5 left-0.5 bg-primary-dark text-white text-[10px] px-1.5 py-0.5 rounded font-medium">
                      Bìa
                    </span>
                  )}
                <button
            ...
            {newImagePreviews.map((preview, index) => (
              <div
                key={`new-${index}`}
                className="relative w-24 h-24 rounded-lg overflow-hidden border-2 border-blue-200 group"
              >
                <img
                  src={preview}
                  alt={`new-${index}`}
                  className="w-full h-full object-cover"
                />
                <span className="absolute bottom-0.5 left-0.5 bg-blue-500 text-white text-[10px] px-1.5 py-0.5 rounded font-medium">
                  Mới
                </span>
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {attr.options?.map((opt) => (
                      <span
                        key={opt.id}
                        className="px-3 py-1 bg-white border border-gray-200 rounded text-sm text-gray-600"
                      >
                        {opt.value}
                      </span>
                    ))}
