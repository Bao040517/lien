  ...
  useEffect(() => {
    const fetchShopData = async () => {
      try {
        ...
        setProducts(
          productsRes.data.data?.content || productsRes.data.data || [],
        );
        setVouchers(
          vouchersRes.data.data?.content || vouchersRes.data.data || [],
        );
      } catch (error) {
  ...
  return (
    <div className="bg-gray-100 min-h-screen pb-10">
      ...
      <div className="container mx-auto px-4 pt-4">
        <Breadcrumb
          items={[{ label: "Trang chủ", path: "/" }, { label: shop.name }]}
        />
      </div>
              ...
              <div className="relative z-10 flex flex-col gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-gray-600 border-2 border-white flex items-center justify-center overflow-hidden">
                    {shop.avatarUrl ? (
                      <img
                        src={getImageUrl(shop.avatarUrl)}
                        alt={shop.name}
            ...
            <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-6 py-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-primary-dark font-medium text-sm">
                    12 tháng trước
                  </div>
                </div>
              </div>
            ...
            <div className="relative group">
              ...
              >
                {vouchers.map((voucher) => (
                  <div
                    key={voucher.id}
                    className="min-w-[280px] md:min-w-[320px] bg-gradient-to-r from-primary-dark to-primary-dark rounded-sm p-0.5 flex shadow-sm snap-start shrink-0 relative"
                  >
                    ...
                    <div className="bg-white w-24 md:w-28 flex flex-col items-center justify-center border-r border-dashed border-gray-200 relative p-2">
                      ...
                      <div className="absolute left-0 top-0 bottom-0 w-1 flex flex-col justify-between overflow-hidden">
                        {[...Array(10)].map((_, i) => (
                          <div
                            key={i}
                            className="w-2 h-2 bg-gray-100 rounded-full -ml-1"
                          ></div>
                        ))}
                      </div>
                    </div>
