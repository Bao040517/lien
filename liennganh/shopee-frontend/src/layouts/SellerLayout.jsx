...
const SellerLayout = () => {
  ...
  const checkShopSetup = async () => {
    try {
      ...
      console.error("Error checking shop setup", e);
    }
  };
  ...
  const fetchBannedCount = async () => {
    try {
      ...
      const data =
        res.data.data?.content || res.data.data || res.data.result || [];
      const banned = Array.isArray(data)
        ? data.filter((p) => p.productStatus === "BANNED" || p.banned).length
        : 0;
      setBannedProductCount(banned);
    } catch (e) {
      console.error("Error fetching banned count", e);
    }
  };
  ...
  const fetchMsgUnreadCount = async () => {
    try {
      ...
      console.error("Error fetching msg unread count", e);
    }
  };
  ...
  const handleMarkRead = async (notif) => {
    ...
    if (
      notif.type === "Product" ||
      notif.type === "PRODUCT_BAN" ||
      notif.type === "PRODUCT_UNBAN"
    ) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-yellow-50">
        <div className="text-center max-w-md">
          ...
          <p className="text-yellow-700 mb-6">
            Yêu cầu đăng ký Seller của bạn đã được gửi và đang được Admin xem
            xét. Vui lòng trở lại sau.
          </p>
          <Link
            to="/"
      ...
      <div className="min-h-screen flex items-center justify-center bg-red-50">
        <div className="text-center max-w-md">
          ...
          <p className="text-red-700 mb-6">
            Yêu cầu trở thành Seller của bạn đã bị từ chối. Vui lòng liên hệ hỗ
            trợ.
          </p>
          <Link
            to="/"
      ...
      <div className="min-h-screen flex items-center justify-center bg-primary-lighter">
        <div className="text-center max-w-md p-8 bg-white rounded-2xl shadow-xl border border-primary-light">
          ...
          <p className="text-gray-500 mb-8 leading-relaxed">
            ...
            Đăng ký ngay hôm nay để bắt đầu kinh doanh và tiếp cận hàng triệu
      ...
      <aside className="w-64 bg-white border-r flex flex-col fixed h-full z-40 shadow-sm">
        ...
        <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            const showBannedBadge =
              item.path === "/seller/products" && bannedProductCount > 0;
            const showNotifBadge =
              item.path === "/seller/notifications" && unreadCount > 0;
            const showMsgBadge =
              item.path === "/seller/messages" && msgUnreadCount > 0;
            return (
              <Link
                key={item.path}
                to={item.path}
        ...
        <div className="border-t p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 bg-primary-dark rounded-full overflow-hidden flex items-center justify-center text-white text-sm font-bold">
              {shopProfile?.avatarUrl ? (
                <img
                  src={getImageUrl(shopProfile.avatarUrl)}
                  alt="Shop avatar"
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium text-gray-800 truncate">
                {shopProfile?.name || user.username}
              </p>
          ...
          <div className="flex gap-2">
            <Link
              to="/"
              className="text-xs text-gray-400 hover:text-primary-dark transition"
            >
              Về cửa hàng
            </Link>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in">
            <div className="bg-primary-dark p-6 text-center text-white">
              ...
              <p className="text-primary-light mt-2 text-sm">
                Yêu cầu đăng ký Kênh Người Bán của bạn đã được duyệt. Hãy đặt
                tên cho Cửa hàng của bạn để bắt đầu kinh doanh.
              </p>
            </div>
