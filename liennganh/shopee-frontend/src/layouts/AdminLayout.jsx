...
const AdminLayout = () => {
        ...
        const fetchBadges = async () => {
            try {
                const [statsRes, msgRes] = await Promise.all([
                    ...
                    api.get('/messages/unread-count', { params: { userId: user.id } })
                ]);
                const stats = statsRes.data.data || statsRes.data;
                const msgUnread = msgRes.data.data || 0;
                const currentCounts = {
                ...
                if (!baseline) {
                    ...
                    setBadges({ '/admin/users': 0, '/admin/sellers': 0, '/admin/products': 0, '/admin/orders': 0, '/admin/messages': msgUnread });
                } else {
                    const deltas = {};
                    ...
                    deltas['/admin/messages'] = msgUnread;
                    setBadges(deltas);
                }
            } catch (e) {
    ...
    const menuItems = [
        ...
        { path: '/admin/sliders', icon: ImageIcon, label: 'Sliders' },
        { path: '/admin/messages', icon: MessageCircle, label: 'Tin nhắn' },
    ];
    ...
    return (
        <div className="min-h-screen flex bg-gray-100">
            ...
            <aside className="w-64 bg-slate-900 text-white flex flex-col fixed h-full z-40">
                ...
                {/* Navigation */}
                <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
